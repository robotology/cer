...
{    
    ...
    if(audio_is_playing)
    {
        ...
        m_rpc_right.write(motion1);
        m_rpc_left.write(motion2);
        Time::delay(3.0);

        audio_is_playing = (unsigned int)player_status->get(1).asInt64() > 0;
        if(audio_is_playing)
        {
            Bottle motion3{"ctpq time 2.0 off 0 pos (29.44 11.60 25.40 70.14 63.08 0 -0.11 0.07)"};
            Bottle motion4{"ctpq time 2.0 off 0 pos (29.36 9.05 -10.02 50.27 32.8217 0 -0.02 -0.36)"};
            m_rpc_right.write(motion3);
            m_rpc_left.write(motion4);
            Time::delay(2.0);
        }
