...
#ifndef ROBOT_MODEL_IMPL_H___
#define ROBOT_MODEL_IMPL_H___
...
namespace cer
{
	namespace robot_model
	{
		...
		public:
			...
			virtual const Transform& getHandTransform(int hand)
			{
				return mHand[hand]->Toj;
			}

			void handL(double& px, double& py, double& pz, double& rx, double& ry, double& rz)
			{
				Transform& T = mHand[L]->Toj;

				px = T.Pj().x; py = T.Pj().y; pz = T.Pj().z;

				Vec3 rpy = T.Rj().rpy();

				rx = rpy.x; ry = rpy.y; rz = rpy.z;
			}

			void handR(double& px, double& py, double& pz, double& rx, double& ry, double& rz)
			{
				Transform& T = mHand[R]->Toj;
