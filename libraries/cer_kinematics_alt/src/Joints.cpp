...
/*
void Trifid2::calcPostureUns(Matrix& q,Transform& Tprec,Component *from,Vec3* Poj_1,Vec3* Zoj_1,Vec3* Voj_1)
{
	Tparent=Tprec;
	...
	{		
		...
		R(0,0)=Ux*Ux*(1.0-cosT)+cosT; R(0,1)=Ux*Uy*(1.0-cosT);      R(0,2)= Uy*sinT;
		R(1,0)= R(0,1);               R(1,1)=Uy*Uy*(1.0-cosT)+cosT; R(1,2)=-Ux*sinT;
		R(2,0)=-R(0,2);               R(2,1)=-R(1,2);               R(2,2)=    cosT;

		double m0=(L/cosT)*(-0.5*R(0,0)+1.5*R(1,1));
		...
		double dNdq1=(2.0/n)*(2.0*q1-q2-q3);
		double dNdq2=(2.0/n)*(2.0*q2-q3-q1);
		double dNdq3=(2.0/n)*(2.0*q3-q1-q2);

		Matrix dRdq1(3,3),dRdq2(3,3),dRdq3(3,3);

		dRdq1(0,2)=(  -2.0*n-N.x*dNdq1)/(n*n); // d(Nx/N)/dq1
		dRdq2(0,2)=(       n-N.x*dNdq2)/(n*n); // d(Nx/N)/dq2
		dRdq3(0,2)=(       n-N.x*dNdq3)/(n*n); // d(Nx/N)/dq3

		dRdq1(1,2)=(        -N.y*dNdq1)/(n*n); // d(Ny/N)/dq1
		dRdq2(1,2)=(-SQRT3*n-N.y*dNdq2)/(n*n); // d(Ny/N)/dq2
		dRdq3(1,2)=( SQRT3*n-N.y*dNdq3)/(n*n); // d(Ny/N)/dq3

		dRdq1(2,2)=(        -N.z*dNdq1)/(n*n); // dCosT/dq1
		dRdq2(2,2)=(        -N.z*dNdq2)/(n*n); // dCosT/dq2
		dRdq3(2,2)=(        -N.z*dNdq3)/(n*n); // dCosT/dq3
		...
		if (Nx2Ny2>0.0)
		{
			double dNxNydq1=(1.0/(Nx2Ny2*Nx2Ny2))*(Nx2Ny2*(N.x*(SQRT3* 0.0)+N.y*(-2.0))-4.0*N.x*N.y*(2.0*q1-q2-q3));
			double dNxNydq2=(1.0/(Nx2Ny2*Nx2Ny2))*(Nx2Ny2*(N.x*(SQRT3*-1.0)+N.y*( 1.0))-4.0*N.x*N.y*(2.0*q2-q3-q1));
			double dNxNydq3=(1.0/(Nx2Ny2*Nx2Ny2))*(Nx2Ny2*(N.x*(SQRT3* 1.0)+N.y*( 1.0))-4.0*N.x*N.y*(2.0*q3-q1-q2));

			dRdq1(1,0)=dRdq1(0,1)=(cosT-1.0)*dNxNydq1-Ux*Uy*dRdq1(2,2);
			dRdq2(1,0)=dRdq2(0,1)=(cosT-1.0)*dNxNydq2-Ux*Uy*dRdq2(2,2);
			dRdq3(1,0)=dRdq3(0,1)=(cosT-1.0)*dNxNydq3-Ux*Uy*dRdq3(2,2);

			double dUx2dq1=(1.0/(Nx2Ny2*Nx2Ny2))*(N.x*N.x*2.0*N.y*SQRT3*( 0.0)-N.y*N.y*2.0*N.x*(-2.0));
			double dUx2dq2=(1.0/(Nx2Ny2*Nx2Ny2))*(N.x*N.x*2.0*N.y*SQRT3*(-1.0)-N.y*N.y*2.0*N.x*( 1.0));
			double dUx2dq3=(1.0/(Nx2Ny2*Nx2Ny2))*(N.x*N.x*2.0*N.y*SQRT3*( 1.0)-N.y*N.y*2.0*N.x*( 1.0));

			double dUy2dq1=-dUx2dq1;
			double dUy2dq2=-dUx2dq2;
			double dUy2dq3=-dUx2dq3;

			dRdq1(0,0)=dUx2dq1+dUy2dq1*cosT+Uy*Uy*dRdq1(2,2);
			dRdq2(0,0)=dUx2dq2+dUy2dq2*cosT+Uy*Uy*dRdq2(2,2);
			dRdq3(0,0)=dUx2dq3+dUy2dq3*cosT+Uy*Uy*dRdq3(2,2);

			dRdq1(1,1)=dUy2dq1+dUx2dq1*cosT+Ux*Ux*dRdq1(2,2);
			dRdq2(1,1)=dUy2dq2+dUx2dq2*cosT+Ux*Ux*dRdq2(2,2);
			dRdq3(1,1)=dUy2dq3+dUx2dq3*cosT+Ux*Ux*dRdq3(2,2);

			Matrix Rt=R;
			//dRdq3.dump();
			//printf("\n");

			Matrix S1=dRdq1*Rt.t();
			Matrix S2=dRdq2*Rt.t();
			Matrix S3=dRdq3*Rt.t();
			...
			double dm0dq1=(L/cosT)*(-0.5*dRdq1(0,0)+1.5*dRdq1(1,1))-(L/(cosT*cosT))*dRdq1(2,2)*(-0.5*R(0,0)+1.5*R(1,1));
			double dm0dq2=(L/cosT)*(-0.5*dRdq2(0,0)+1.5*dRdq2(1,1))-(L/(cosT*cosT))*dRdq2(2,2)*(-0.5*R(0,0)+1.5*R(1,1));
			double dm0dq3=(L/cosT)*(-0.5*dRdq3(0,0)+1.5*dRdq3(1,1))-(L*(cosT*cosT))*dRdq3(2,2)*(-0.5*R(0,0)+1.5*R(1,1));

			V[0]=Vec3(0.0,0.0,1.0)-dm0dq1*R.Ex()-m0*Vec3(dRdq1(0,0),dRdq1(1,0),dRdq1(2,0));
			V[1]=                 -dm0dq2*R.Ex()-m0*Vec3(dRdq2(0,0),dRdq2(1,0),dRdq2(2,0));
			V[2]=                 -dm0dq3*R.Ex()-m0*Vec3(dRdq3(0,0),dRdq3(1,0),dRdq3(2,0));
		}
		else
		{
			Vec3 P1(     L, 0.0        ,q1);
			Vec3 P2(-0.5*L, 0.5*SQRT3*L,q2);
			Vec3 P3(-0.5*L,-0.5*SQRT3*L,q3);

			Z[0]=(P3-P2).norm(2.0/(3.0*L));
			Z[1]=(P1-P3).norm(2.0/(3.0*L));
			Z[2]=(P2-P1).norm(2.0/(3.0*L));

			V[0]=Vec3(0.0,0.0,1.0/3.0);
			V[1]=Vec3(0.0,0.0,1.0/3.0);
			V[2]=Vec3(0.0,0.0,1.0/3.0);
		}
