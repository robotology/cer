...
{
    if(response[0] == REPLY_HEAD)
    {
        return true;
    }

    if(response[0] == ERROR_HEAD)
    {
...
//             printf("> SUCCESS!\n"); fflush(stdout);
            ...
            nbytes_r = readBytes(&buf[2], (int) buf[1]);
//             printf("\tReply is:\n");
...
//     printBuffer(command, command_len);
    ...
    int nbytes_w = ::write(fd_ser, (void*)command, command_len);

    // Read the write reply
    memset(response, 0x00, 20);
    int nbytes_r = readBytes(response, 2);
